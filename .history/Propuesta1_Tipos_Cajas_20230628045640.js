...
d3.csv("pokemon.csv").then(data => {
    ...
    const zoom = d3.zoom()
        .scaleExtent([1, 40]) // this defines the minimum and maximum zoom (1x to 40x here)
        .translateExtent([[0, 0], [width, height]]) // this restricts the panning to the SVG area only
        .extent([[500, 5000], [500+width, 50+height]]) // this sets the initial zoom area to the full SVG area
        .on("zoom", zoomed);

    function zoomed({transform}) {
        // Apply transform on boxes and median lines
        svg.selectAll(".box, .median").attr("transform", transform);
    
        // Update the axes with the new transformed domain
