...
d3.csv("pokemon.csv").then(data => {
    ...
    // Step 3: Create a zoom behavior and add it to the boxplot group
    const zoom = d3.zoom()
        .scaleExtent([1, 40])
    ...
    const boxplotRect = boxplot.append("rect")
        .attr("width", width)
        .attr("height", height)
        .style("fill", "none")
        .style("pointer-events", "all");
    ...
    const updateBoxPlot = (selectedType, xScale = x, yScale = y) => {
        ...
        svg.selectAll(".box")
            ...
            .attr("height", d => yScale(d3.quantile(d, 0.25)) - yScale(d3.quantile(d, 0.75)));
    ...
    boxplotRect.on("dblclick.zoom", () => {
        boxplotRect.transition().duration(750).call(zoom.transform, d3.zoomIdentity);
    });
