d3.csv("pokemon.csv").then(data => {
    ...
    // Add this somewhere near your other scale definitions
    const genScale = d3.scaleLinear()
    .domain([1, 8]) // Assumes you have 8 generations of Pokemon
    .range([0, width]);

    // Create the brush
    const brush = d3.brushX()
    .extent([[0, 0], [width, 30]]) // 30 is the height of the brush area
    .on("end", updateChart);

    // Add a new SVG to hold the brush
    const brushSvg = d3.select("#boxplot")
    .append("svg")
    .attr("width", width + margin.left + margin.right)
    .attr("height", 50) // Height of the brush SVG
    .append("g")
    .attr("transform", `translate(${margin.left},${margin.top})`)
    .call(brush);
