d3.csv("pokemon.csv").then(data => {
    ...
    const generationScale = d3.scaleBand()
        .domain(data.map(d => d["Generation"])) // assuming "Generation" is the field in your data
    ...
    function brushed(event) {
        if (event.selection) {
            let [x0, x1] = event.selection.map(d => generationScale.invert(d));
            // filter your data based on the brush selection here
            // For example:
            let filteredData = data.filter(d => d["Generation"] >= x0 && d["Generation"] <= x1);
            updateBoxPlot(filteredData, types[0]);
        }
    }

    function brushended(event) {
        if (!event.selection) {
            // if there is no selection, reset the brush
