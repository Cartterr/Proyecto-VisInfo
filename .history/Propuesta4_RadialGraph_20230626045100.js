...
let allStats = ['HP', 'Attack', 'Defense', 'Sp_Atk', 'Sp_Def', 'Speed'];
// Load the data
d3.csv('pokemon.csv').then(data => {
    ...
    allStats.forEach(stat => {
        ...
        for (let i = 50; i <= 255; i += 50) {
            ...
            svg.append('text')
                .attr('x', d => radialScale(i) * Math.cos(angleScale(stat) - Math.PI / 2))
                .attr('y', d => radialScale(i) * Math.sin(angleScale(stat) - Math.PI / 2)) // Positioning labels slightly above the circle
                .attr('text-anchor', 'middle')
                .attr('font-size', '10px')
                .attr('fill', 'white')
                .text(i);
    ...
    function drawPath(pokemon, color) {
        ...
        points.forEach((point, i) => {
            svg.append('circle')
                ...
                .attr('r', 5)
                .attr('fill', color)
                .attr('id', 'circle_' + sanitizedPokemonName + '_' + allStats[i]); // Use the underscored stat names
        });
    }
    ...
    function addPokemonToGraph(pokemon) {
        ...
        if (!existingPath.empty()) {
            existingPath.remove();
            allStats.forEach(stat => {
                svg.select('#circle_' + sanitizedPokemonName + '_' + stat).remove(); // Use the underscored stat names
            });
