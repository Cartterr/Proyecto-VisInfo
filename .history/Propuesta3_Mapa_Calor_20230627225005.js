...
d3.csv("typing_chart.csv").then(function(data) {
     ...
     function updateHistogram(data, colorScale) {
          x.domain([d3.min(data, function(d) { return +d.Total; }), d3.max(data, function(d) { return +d.Total; })]);
          var bins = d3.histogram().domain(x.domain()).thresholds(x.ticks(20))(data.map(function(d) { return +d.Total; }));
          y.domain([0, d3.max(bins, function(d) { return d.length; })]);
     
          var bar = histogramSvg.selectAll(".bar").data(bins);
     ...
     function brushed() {
          ...
          updateHistogram(filteredData, colorScale);
     }
     ...
     updateHistogram(data, colorScale);
});
