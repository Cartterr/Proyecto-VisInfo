...
d3.csv("pokemon.csv").then(data => {
    ...
    let root = d3.hierarchy({ values: nestedData }, d => d.values)
    .sum(d => +d.data["Total"])
    ...
    .style("fill", d => color(d.parent.data.key)) // color based on Type 1
    .on("click", function() {
        const selectedType = d3.select(this).data()[0].parent.data.key; // select based on Type 1
        updateBoxPlot(selectedType);
    });
    ...
    .attr("x", d => d.x0+5)   
    .attr("y", d => d.y0+20)
    .text(d => d.data.key) // display Type 2
    .attr("font-size", "15px")
    .attr("fill", "white");
