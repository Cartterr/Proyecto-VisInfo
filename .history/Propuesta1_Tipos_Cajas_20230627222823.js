d3.csv("pokemon.csv").then(data => {
    ...
    function updateBoxPlot(selectedData, selectedType) {
        const typeData = selectedData.filter(d => d["Type 1"] === selectedType);
    ...
    const cumulativeData = generations.map(g => ({
        generation: g,
        count: data.filter(d => +d.Generation === g).length
    }));

    const genArea = d3.area()
        .x(d => genX(d.generation))
        .y0(genHeight)
        .y1(d => genY(d.count));

    // Replace the bar chart with an area chart
    genSvg.append("path")
    ...
    genSvg.selectAll(".label")
        .data(cumulativeData)
        .enter()
        .append("text")
        .attr("class", "label")
        .attr("x", d => genX(d.generation) + genX.bandwidth() / 2)
        .attr("y", d => genY(d.count) - 5)
        .text(d => d.count);
