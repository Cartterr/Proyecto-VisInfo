...
d3.csv("pokemon.csv").then(data => {
    ...
    // Create a data structure that reflects the hierarchy of your data
    const typeHierarchy = d3.stratify()
        .id(d => d["Type 2"])
        .parentId(d => d["Type 1"])
        (data);
    ...
    // Use this root to draw your rectangles
    const leaves = svgTreemap.selectAll('rect')
        .data(root.leaves())
        .enter()
        .append('rect')
            .attr('x', d => d.x0)
            ...
            .style("fill", d => color(d.parent.data.id)) // color based on Type 1
            .on("click", function() {
                const selectedType = d3.select(this).data()[0].parent.data.id; // select based on Type 1
                updateBoxPlot(selectedType);
            });
    ...
    svgTreemap.selectAll("text")
        ...
        .append("text")
            .attr("x", d => d.x0+5)   
            .attr("y", d => d.y0+20)
            .text(d => d.data.id) // display Type 2
            .attr("font-size", "15px")
            .attr("fill", "white");
