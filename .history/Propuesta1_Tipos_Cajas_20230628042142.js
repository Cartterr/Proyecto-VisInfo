...
d3.csv("pokemon.csv").then(data => {
   ...
   //TREEMAP
    ...
    // after defining updateBoxPlot

    function brushed() {
        if (d3.event.sourceEvent.type === "brush" || d3.event.sourceEvent.type === "end") {
            const selection = d3.event.selection;
            x.domain(selection.map(xBrush.invert, xBrush));
            y.domain(selection.map(yBrush.invert, yBrush));
            drawBoxPlot(x, y);
        }
    }

    svg.call(d3.zoom().scaleExtent([1, Infinity]).on("zoom", zoomed));

    function zoomed() {
        if (d3.event.sourceEvent.type === "zoom") {
            x.domain(d3.event.transform.rescaleX(xBrush).domain());
            y.domain(d3.event.transform.rescaleY(yBrush).domain());
            drawBoxPlot(x, y);
        }
    }
});