...
d3.csv("pokemon.csv").then(data => {
    ...
    let zoom = d3.zoom()
        .scaleExtent([0.5, 20])  // The scale extent restricts zoom out and in.
        .on("zoom", zoomed);
    ...
    // Then, within the zoomed function:
    function zoomed(event) {
        ...
        let updatedYScale = transform.rescaleY(y);

        // Now apply the updated scales to your boxplots, lines, and other SVG elements
        // Here's an example of how you'd do it with your boxes and median lines:

        svg.selectAll(".box")
            .attr("x", (d, i) => updatedXScale(statNames[i]) + (updatedXScale.bandwidth() - boxWidth) / 2)
            .attr("width", boxWidth)
            .attr("y", d => updatedYScale(d3.quantile(d, 0.75)))
            .attr("height", d => updatedYScale(d3.quantile(d, 0.25)) - updatedYScale(d3.quantile(d, 0.75)));

        svg.selectAll(".median")
            .attr("x1", (d, i) => updatedXScale(statNames[i]) + (updatedXScale.bandwidth() - boxWidth) / 2)
            .attr("x2", (d, i) => updatedXScale(statNames[i]) + (updatedXScale.bandwidth() + boxWidth) / 2)
            .attr("y1", d => updatedYScale(d3.median(d)))
            .attr("y2", d => updatedYScale(d3.median(d)));

        // Update axes
        svg.select(".x-axis").call(xAxis.scale(updatedXScale));
        svg.select(".y-axis").call(yAxis.scale(updatedYScale));
    }
