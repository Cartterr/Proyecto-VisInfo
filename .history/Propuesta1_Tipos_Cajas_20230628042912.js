...
d3.csv("pokemon.csv").then(data => {
    ...
    function brushed(event) {
        if (event.selection) {
            let e = event.selection;
            let newDomain = [x.invert(e[0]), x.invert(e[1])];
            x.domain(newDomain);
            svg.select(".x-axis").call(xAxis);
            updateBoxPlot(selectedType, newDomain);
        }
    }
    ...
    function zoomed(event) {
        ...
        let newDomain = t.rescaleX(x2).domain();
        x.domain(newDomain);
        svg.select(".x-axis").call(xAxis);
        updateBoxPlot(selectedType, newDomain);
    }
    ...
    const updateBoxPlot = (selectedType, zoomDomain = null) => {
        let typeData;
        ...
        // Only change the domain of x to statNames if zoomDomain is not provided
        if (!zoomDomain) {
            x.domain(statNames);
        }
        y.domain([0, d3.max(typeStats, d => d3.max(d))]);
