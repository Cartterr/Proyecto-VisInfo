...
d3.csv('pokemon.csv').then(data => {
    ...
    allStats.forEach(stat => {
        ...
        svg.append('text')
            .attr('x', d => (radialScales[stat](statDomains[stat][1]) + 10) * Math.cos(angleScale(stat) - Math.PI / 2))
            .attr('y', d => (radialScales[stat](statDomains[stat][1]) + 10) * Math.sin(angleScale(stat) - Math.PI / 2))
        ...
        [0.25, 0.5, 0.75, 1].forEach((percentage) => {
            ...
            svg.append('text')
                .attr('x', d => radialScales[stat](value) * Math.cos(angleScale(stat) - Math.PI / 2))
                .attr('y', d => radialScales[stat](value) * Math.sin(angleScale(stat) - Math.PI / 2)) // Positioning labels slightly above the circle
                .attr('text-anchor', 'middle')
                .attr('font-size', '10px')
                .attr('fill', 'white')
