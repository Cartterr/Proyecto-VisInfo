...
d3.csv('pokemon.csv').then(data => {
    ...
    let sanitizedPokemonName = sanitizeName(pokemon.Name);
    ...
    function addPokemonToGraph(pokemon) {
        ...
        if (!existingPath.empty()) {
            ...
            d3.select('#button_' + sanitizedPokemonName).classed('activePokemon', false);
            activePokemonSet.delete(pokemon.Name);
        } else {
            let color = generateColor();
            drawPath(pokemon, color);
            addPokemonToList(pokemon);
            d3.select('#button_' + sanitizedPokemonName).classed('activePokemon', true);
            activePokemonSet.add(pokemon.Name);
        }
    }
